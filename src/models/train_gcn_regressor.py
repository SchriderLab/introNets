...
def main():
    ...
    generator = GCNRegDataGeneratorH5(args.ifile,
    ...
    for ix in range(int(args.n_epochs)):
        ...
        for ij in range(int(args.n_steps)):
            ...
            if (ij + 1) % 5 == 0:
                logging.info(
                    'root: Epoch {0}, step {2}: got loss of {1}, lr: {3}'.format(ix, np.mean(losses),
                                                                                   ij + 1, lr_scheduler.get_last_lr()))
        ...
        logging.info(
            'root: Epoch {0}, got val loss of {1}'.format(ix, val_loss))

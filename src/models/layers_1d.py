...
class GCNConvNet_beta(nn.Module):
    def __init__(self, in_channels = 1, depth = 7, 
        ...
        for ix in range(depth):
            ...
            self.gcns.append(GATConv(sites, sites, heads = out_channels, edge_dim = 8))
...
class GCNEqRegressor_beta(nn.Module):
    def __init__(self, sites = 128, pred_pop = 1, n_layers = 4):
        super(GCNEqRegressor_beta, self).__init__()
        self.pred_pop = pred_pop
        ...
        self.out_dim = in_channels * n_sites
    ...
    def forward(self, x, edge_index, edge_attr, batch, return_intermediates = False):
        ...
        for ix in range(len(self.down)):
            x = self.norms_down[ix](self.down[ix](x))
...
class GCNUNet_psi(nn.Module):
    def __init__(self, n_layers = 3, sites = 128, 
        ...
        super(GCNUNet_psi, self).__init__()
    ...
    def forward(self, x, edge_index, edge_attr, batch, return_intermediates = False):
        ...
        batch_size, channels, ind, sites = x.shape
        xg = x.transpose(1, 2).flatten(0, 1).transpose(1, 2)
...
class GCNUNet_theta(nn.Module):
    def __init__(self, n_layers = 3, sites = 128, 
        ...
        self.pre_out = Res1dBlock((in_channels + 16 + up_channels[-1] * 2, ), in_channels + 16 + up_channels[-1] * 2, 1, pooling = None)
        self.pre_out_gru = nn.GRU(in_channels + 16 + up_channels[-1] * 2, in_channels + 16 + up_channels[-1] * 2, batch_first = True, bidirectional = True)
        
        self.out = nn.Conv2d((in_channels + 16 + up_channels[-1] * 2) * 3, 1, 1, 1, bias = False)
